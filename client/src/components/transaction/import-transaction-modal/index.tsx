...
const ImportTransactionModal = () => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <Button
        variant="outline"
        className="!shadow-none !cursor-pointer !border-[#002B4C] !text-[#002B4C] !bg-white flex items-center gap-2"
        onClick={() => setOpen(true)}
      >
        <ImportIcon className="!w-5 !h-5 text-[#14A0C4]" />
        Bulk Import
      </Button>
      <DialogContent className="max-w-2xl min-h-[40vh] bg-white p-6 rounded-lg shadow-md">
        {renderStep()}
      </DialogContent>
    </Dialog>
  );
