...
const ReceiptScanner = ({
  ...
  const handleReceiptUpload = async (
    ...
    reader.onload = (e) => {
      ...
      aiScanReceipt(formData)
        .unwrap()
        .then((res: { data: AIScanReceiptData }) => {
          updateProgress(100);
  ...
  return (
    <div className="space-y-3 bg-white p-2 rounded-md shadow-sm">
      <Label className="text-sm font-medium text-[#002B4C]">AI Scan Receipt</Label>
          ...
          {!loadingChange ? (
            <>
              <Input
                ...
                className="max-w-[250px] px-1 h-9 cursor-pointer text-sm
                  file:mr-2 file:rounded file:border-0 file:bg-[#14A0C4] file:px-3 file:py-px
                  file:text-sm file:font-medium file:text-white hover:file:bg-[#14A0C4]/90"
                disabled={loadingChange}
              />
              <p className="mt-2 text-[11px] px-2 text-[#333333]">
                JPG, PNG up to 5MB
              </p>
            </>
          ) : (
