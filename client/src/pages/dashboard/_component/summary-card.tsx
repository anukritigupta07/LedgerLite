...
const getCardStatus = (
  ...
  if (cardType === "savings") {
    ...
    if (value < 10) {
      return { label: "Low Savings", color: "text-red-500", Icon: TrendingDownIcon, description: `Only ${value.toFixed(1)}% saved` };
    }
    if (value < 20) {
      return { label: "Moderate", color: "text-orange-400", Icon: TrendingDownIcon, description: `${expenseRatio?.toFixed(0)}% spent` };
    }
    if (expenseRatio && expenseRatio > 75) {
      return { label: "High Spend", color: "text-red-500", Icon: TrendingDownIcon, description: `${expenseRatio.toFixed(0)}% spent` };
    }
    if (expenseRatio && expenseRatio > 60) {
      return { label: "Warning: High Spend", color: "text-yellow-500", Icon: TrendingDownIcon, description: `${expenseRatio.toFixed(0)}% spent` };
    }
    return { label: "Good Savings", color: "text-green-500", Icon: TrendingUpIcon };
  }
  ...
  if (cardType === "balance" && value < 0) {
    return { label: "Overdrawn", color: "text-red-500", Icon: TrendingDownIcon, description: "Balance is negative" };
  }
...
const SummaryCard: FC<SummaryCardProps> = ({
  ...
  return (
    <Card className="bg-white border border-gray-200 shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 !pb-5">
        <CardTitle className="text-[15px] font-semibold text-[#002B4C]">{title}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className={cn("text-4xl font-bold", cardType === "balance" && value < 0 ? "text-red-500" : "text-[#333333]")}>
          <CountUp start={0} end={value} preserveValue decimals={2} decimalPlaces={2} formattingFn={formatCountupValue} />
        </div>

        <div className="text-sm text-[#333333] mt-2">
          {cardType === "savings" ? (
            <div className="flex items-center gap-1.5">
              <status.Icon className={cn("size-3.5", status.color)} />
              <span className={status.color}>{status.label} {value !== 0 && `(${formatPercentage(value)})`}</span>
              {status.description && <span className="text-gray-400 ml-1">• {status.description}</span>}
            </div>
            ...
            <div className="flex items-center gap-1.5">
              {percentageChange !== 0 && (
                <div className={cn("flex items-center gap-0.5", trendDirection === "positive" ? "text-[#14A0C4]" : "text-red-500")}>
                  {trendDirection === "positive" ? <TrendingUpIcon className="size-3" /> : <TrendingDownIcon className="size-3" />}
                  <span>
                    {formatPercentage(percentageChange || 0, { showSign: percentageChange !== 0, isExpense: cardType === "expenses", decimalPlaces: 1 })}
                  </span>
                </div>
              )}
              {percentageChange === 0 && (
